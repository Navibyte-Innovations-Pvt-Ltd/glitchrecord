# GlitchRecord Extension API

GlitchRecord extensions run in the editor renderer and use a permission-gated host API. They can draw into the render pipeline, react to playback and export events, register cursor effects, add settings panels, and contribute packaged assets such as frames, wallpapers, and cursor styles.

## Quick Start

For local user-installed extensions, use `Extensions -> Open Directory` in the app. GlitchRecord stores them in the app `userData/extensions` directory.

### Minimum Extension

```text
my-extension/
  glitchrecord-extension.json
  index.js
```
...
Extensions can be authored in TypeScript. Import the `GlitchRecordExtensionAPI` type from `types.ts` in this repo for full IDE auto-complete and compile-time checks:

```ts
import type { GlitchRecordExtensionAPI } from "./types";

export function activate(api: GlitchRecordExtensionAPI) {
...
GlitchRecord loads the `main` entry from the manifest, which must be a `.js` file. If you author in TypeScript, compile or bundle to JavaScript before packaging. A `tsconfig.json` with `"module": "ESNext"` and `"target": "ESNext"` works well since extensions run in a Chromium renderer.

## Manifest

`glitchrecord-extension.json` is validated when the extension loads.
...
| `engine` | `string` | No | Minimum supported GlitchRecord version |
...
`contributes` is metadata only today. GlitchRecord does not auto-register runtime behavior from the manifest. Use `activate()` to call APIs such as `registerFrame()`, `registerWallpaper()`, `registerCursorStyle()`, `registerSettingsPanel()`, and `playSound()`.
...
- `post-webcam`, `post-annotations`, and `final` run after GlitchRecord restores the canvas transform. Use `sceneTransform` manually if you want those overlays to move with the scene.
...
Extensions can draw icons from GlitchRecord's bundled Phosphor icon set directly on a canvas context:
...
1. Discovery: GlitchRecord scans built-in extensions and the user extensions directory.
...
- `packages/glitchrecord-extension` in the glitchgrab monorepo — the Glitchgrab Script Generator panel (permissions: `timeline`, `ui`, `cursor`).
