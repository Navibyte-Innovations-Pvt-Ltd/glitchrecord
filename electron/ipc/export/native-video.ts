...
const NVIDIA_CUDA_EXPORT_ENV = "GLITCHRECORD_EXPERIMENTAL_NVIDIA_CUDA_EXPORT";
const NVIDIA_CUDA_ALLOW_AUDIO_EXPORT_ENV = "GLITCHRECORD_NVIDIA_CUDA_ALLOW_AUDIO_EXPORT";
const NVIDIA_CUDA_FORCE_VIDEO_ONLY_ENV = "GLITCHRECORD_NVIDIA_CUDA_FORCE_VIDEO_ONLY";
const NVIDIA_CUDA_AUTO_STALL_TIMEOUT_ENV = "GLITCHRECORD_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS";
const DEFAULT_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS = 120_000;
const NATIVE_GPU_STALL_TIMEOUT_ENV = "GLITCHRECORD_NATIVE_GPU_STALL_TIMEOUT_MS";
...
function shouldPersistNativeExportDiagnostics() {
	const rawValue = process.env.GLITCHRECORD_NATIVE_EXPORT_DIAGNOSTICS?.trim().toLowerCase();
...
function shouldPersistNvidiaCudaExportDiagnostics() {
	return (
		...
		process.env.GLITCHRECORD_NVIDIA_CUDA_EXPORT_DIAGNOSTICS === "1"
	);
}
...
async function resolveExperimentalWindowsGpuExporterPath() {
	...
	const executableNames = ["glitchrecord-gpu-export.exe", "gpu-export-probe.exe"];
	const candidates: string[] = [];
	const configuredPath = process.env.GLITCHRECORD_WINDOWS_GPU_EXPORT_EXE;
	if (configuredPath) {
...
export async function resolveExperimentalNvidiaCudaExportScriptPath() {
	...
	const candidates: string[] = [];
	const configuredPath = process.env.GLITCHRECORD_NVIDIA_CUDA_EXPORT_SCRIPT;
	if (configuredPath) {
		candidates.push(configuredPath);
	}
	const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
...
function resolveExperimentalNvidiaCudaNodeCommand() {
	const configuredNodePath = process.env.GLITCHRECORD_NVIDIA_CUDA_NODE_EXE;
...
export function buildExperimentalNvidiaCudaStaticLayoutArgs(
	...
	const args = [
		...
		"--stream-sync",
		"--prewarm-ms",
		process.env.GLITCHRECORD_NVIDIA_CUDA_PREWARM_MS || "500",
		"--content-x",
		String(Math.round(options.offsetX)),
		"--content-y",
		String(Math.round(options.offsetY)),
	...
	if (process.env.GLITCHRECORD_NVIDIA_CUDA_SAMPLE_GPU === "1") {
...
async function runExperimentalNvidiaCudaStaticLayoutExport(
	...
	const env = {
		...process.env,
		...nodeCommand.env,
		GLITCHRECORD_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY:
			process.env.GLITCHRECORD_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY ?? "1",
		GLITCHRECORD_FFMPEG_EXE: ffmpegPath,
		GLITCHRECORD_FFPROBE_EXE: process.env.GLITCHRECORD_FFPROBE_EXE ?? getFfprobeBinaryPath(),
...
export async function exportNativeStaticLayoutVideo(
	...
	const sessionId =
		options.sessionId ??
		`glitchrecord-static-layout-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export async function probeNativeVideoEncoder(
	...
	const outputPath = path.join(
		app.getPath("temp"),
		`glitchrecord-export-probe-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
...
export async function muxNativeVideoExportAudio(
	...
	if (audioMode === "edited-track" && !useEditedTrackFiltergraph) {
		...
		audioInputPath = path.join(
			app.getPath("temp"),
			`glitchrecord-export-audio-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${extension}`,
		);
...
export async function muxExportedVideoAudioBuffer(
	...
	const tempVideoPath = path.join(
		app.getPath("temp"),
		`glitchrecord-export-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
	const metrics: NativeVideoAudioMuxMetrics = {};
	let succeeded = false;
